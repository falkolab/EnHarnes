...
- **Monthly (upstream template):** in a fork of this template, run the upstream sync — `make upstream-check` to look without acting. It reconciles the fork with the template: take template fixes in, offer generic local fixes back as a pull request, record what is declined. A `SessionStart` hook reminds when the pass is overdue. Protocol: `.claude/skills/harness.upstream/SKILL.md`.
